...
def load_all():
	models = joblib.load("my_staffing_models.pkl")		# opens trained brains/models
	features = joblib.load("my_feature_columns.pkl")	# opens list of expected columns
	venues = pd.read_csv("Venue Database.csv")		# venue info (capacity, entrances, ect.)
	rates = pd.read_csv("Local Rates Database")		# local hourly wages
...
if st.button("Calculate Staffing & Costs"):
	...
	entrances = wenue_row["num_entrances"]
	...
	df["venue_name" + venue_name.replace(" ", "_")] = 1
